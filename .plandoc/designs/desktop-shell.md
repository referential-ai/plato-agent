...
- Lead closure reviews 2026-07-12/13 (PR #140: R1–R9, C10–C16) are design input.
...
- an uninstall-grade daemon shutdown/control operation — refuses ordinary shutdown while a run or approval is active unless uninstall semantics explicitly choose otherwise.
...
## Sequencing
Each phase is its own issue and PR(s); implementation starts only on a `Ready for dev` phase issue. The three D2 protocol additions are their own daemon issues: phase 1 consumes the typed transcript, phase 2 the pending-approval snapshot, phase 5 the shutdown/control operation.
...
5. **Windows packaging + distribution (R7, C13).** Signed NSIS installer with pinned sidecar; clean-VM install/uninstall and cold launch with an explicitly pre-provisioned workspace/config and user-scoped provider credential; SmartScreen behavior recorded; signature checks on all four artifacts. Uninstall/upgrade consumes the shutdown/control daemon issue: stop every installed-sidecar workspace daemon (wake the listener, remove its lock), remove or replace the executable, and relaunch — no reboot, no forced kill.
