...
describe('desktop chat', () => {
	it('uses the first-launch picker and renders the full typed session history', async () => {
		...
		await expect.element(page.getByLabelText('Platonic')).toBeVisible();
		await expect.element(page.getByRole('heading', { name: 'Select a workspace' })).toBeVisible();
		await page.getByRole('button', { name: 'Choose folder' }).click();
		await expect.element(page.getByPlaceholder('Message Platonic')).toBeVisible();
	...
	it('renders the workspace guard conflict separately from daemon failure', async () => {
		mockIPC((command) => {
			if (command === 'bootstrap') {
				return Promise.reject({
					...
					message: 'This workspace is already open in Platonic'
				});
			}
			throw new Error(`unexpected command ${command}`);
		});
		...
		await expect.element(page.getByText('This workspace is already open in Platonic')).toBeVisible();
	});
	...
	it('replaces live assistant deltas with the committed response without duplication', async () => {
		...
		await expect.element(page.getByRole('region', { name: 'Platonic, responding' })).toHaveTextContent('Draft answer');
		...
		await expect.element(page.getByRole('region', { name: 'Platonic' })).toHaveTextContent('Canonical answer');
